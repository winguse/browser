...
- **Submodules**: Tracks `browser.js` (`https://github.com/HeyPuter/browser.js`), `firefox-wasm` (`https://github.com/HeyPuter/firefox-wasm.git`), and `epoxy-tls` (`https://github.com/MercuryWorkshop/epoxy-tls`, branch `multiplexed`) as Git submodules.
...
- **Static Asset Serving**: `browser.js` static assets (`packages/chrome/dist` and `packages/sandbox`) and `firefox-wasm` static assets (`demo/chrome/dist`) are served directly by the Rust server under `/`, `/sandbox/`, and `/ff/` with SPA fallback routing.
...
├── firefox-wasm/               # Git Submodule: HeyPuter/firefox-wasm
...
2. **Parallel Frontend Asset Build Jobs (`build-browser-assets` & `build-firefox-assets`)**:
   - `build-browser-assets`: Compiles `browser.js` static assets (`packages/chrome/dist` & `packages/sandbox`).
   - `build-firefox-assets`: Installs Clang/LLVM, Emscripten, and Rust toolchains to compile `firefox-wasm` static assets (`demo/chrome/dist`).
   - Both jobs run concurrently on separate `ubuntu-latest` runners.
3. **Parallel Native Multi-Arch Container Build Job (`build`)**:
   ...
   - Downloads pre-built `browser-assets` and `firefox-assets` artifacts.
...
| - | Path to Firefox WASM static assets | `FIREFOX_DIR` | `./dist/firefox` |
...
3. **Static File Server**: Serves `/sandbox/*` from `SANDBOX_DIR`, `/ff/*` from `FIREFOX_DIR`, and all other paths from `STATIC_DIR`. Fallbacks non-extension paths to `index.html` for SPA routing.
